...
* \[x] Centrality + Semantic score 결합 로직
* \[x] Graph pruning (backbone extraction) 알고리즘 구현

  * 계보를 가장 잘 설명하는 핵심 경로만 추출
* \[x] Top-k 논문 리스트 생성
* \[x] 설명 가능한 구조 경로 생성
* \[x] 단위 테스트: 결합 점수, pruning 결과
* \[x] 통합 테스트: Subgraph → Ranking 파이프라인
